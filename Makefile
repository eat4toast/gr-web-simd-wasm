...
gnuradio: build-output/volk build-output/boost build-output/pyqt5 build-output/numpy build-output/qwt
	...
	mkdir -p build-output/gnuradio/_lib
	chmod 777 build-output/gnuradio/_lib
...
webapp: gnuradio-rebuild
	...
	cd /home/shh/gnuradio-web/grc-dev && find . -name "*.py" | xargs ../build-output/cpython/wasm/bin/python3.11-i386 -m compileall
